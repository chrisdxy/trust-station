    ...
    if (editingProject) {
      ...
      try {
        ...
        if (data.success) {
          ...
          setFormData({ title: '', location: '', industry: '', types: [], description: '', coverImage: '', images: [], members: [], isDueDiligence: false, dueDiligenceDetails: '' });
          setShowModal(false);
          toast('项目已更新');
        } else {
    ...
    <Layout>
      ...
      <div className="max-w-6xl mx-auto">
              ...
              {filteredProjects.map((project, idx) => (
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="text-sm text-slate-600 dark:text-slate-400 line-clamp-2 mb-3" dangerouslySetInnerHTML={{ __html: renderDescription(project.description) }} />
        ...
        <AnimatePresence>
          {selectedProject && (
            <>
              <WeChatShareSetup title={selectedProject.title} description={selectedProject.description?.replace(/<[^>]*>/g, '').slice(0, 200)} imageUrl={selectedProject.coverImage || ''} />
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-700 sticky top-0 bg-white dark:bg-slate-800 z-10">
                    ...
                    <div className="flex items-center gap-2">
                      <ShareButton targetType="project" targetId={selectedProject.id} title={selectedProject.title} description={selectedProject.description?.replace(/<[^>]*>/g, '').slice(0, 200) || ''} imageUrl={selectedProject.coverImage || ''} />
                      <button
                        onClick={() => {
                  ...
                  <SharePoster
                    type="project"
                    title={selectedProject.title}
                    description={selectedProject.description?.replace(/<[^>]*>/g, '').slice(0, 100) || ''}
                    coverImage={selectedProject.coverImage || ''}
                    shareUrl={`${window.location.origin}/share/project/${selectedProject.id}?inviter=${currentUserId || ''}`}
                  />
