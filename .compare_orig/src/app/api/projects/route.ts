...
export async function POST(request: NextRequest) {
  try {
    const { userId, title, description, coverImage, projectTypes, industry, location, budget, timeline, teamSize, requirements } = await request.json();
    ...
    await pool.query(
      `INSERT INTO projects (id, user_id, title, name, description, location, industry, types, images, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft')`,
      [id, userId, title, title, description || '', location || '', industry || '',
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { title, description, coverImage, projectTypes, industry, location } = await request.json();
    ...
    if (coverImage !== undefined) { fields.push('images = ?'); values.push(JSON.stringify([coverImage].filter(Boolean))); }
