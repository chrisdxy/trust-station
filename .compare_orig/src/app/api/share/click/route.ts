...
export async function POST(request: NextRequest) {
  try {
    ...
    // 记录点击（含分享者ID用于溯源）
    await pool.query(
      `INSERT INTO share_clicks (share_code, visitor_id, inviter_id, ip_address, user_agent, referer)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [shareCode, visitorId || null, share.user_id || null, ipAddress || null, userAgent || null, referer || null]
    );
