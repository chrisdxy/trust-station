...
export async function GET(request: NextRequest) {
  try {
    ...
    // 映射前端类型到DB值
    const dbType = rawType === 'user' ? 'individual' : rawType;
    ...
    if (dbType) {
      if (dbType === 'coordinator') {
        ...
        where.push('id COLLATE utf8mb4_general_ci IN (SELECT user_id FROM partner_applications)');
      } else {
    ...
    const [rows]: any = await pool.query(
      `SELECT id, display_name, real_name, phone, email, user_type, created_at, updated_at FROM users WHERE ${where.join(' AND ')} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
      [...params, limit, offset]
    );
