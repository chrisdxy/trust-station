...
export async function POST(request: NextRequest) {
  try {
    ...
    await pool.query(
      `INSERT INTO activities (id, user_id, title, description, location, start_time, max_participants, organizer_id, cover_image, is_paid, price, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'upcoming')`,
      [id, userId, title, description || '', location || '', startTime, maxParticipants || null, organizerId, coverImage || null, isPaid, price]
    );
