...
export async function GET(
  ...
  try {
    ...
    try { related_items = row.related_items ? (typeof row.related_items === 'string' ? JSON.parse(row.related_items) : row.related_items) : []; } catch { related_items = []; }
    try { related_parties = row.related_parties ? (typeof row.related_parties === 'string' ? JSON.parse(row.related_parties) : row.related_parties) : []; } catch { related_parties = []; }
